...
abbr -a ff 'fastfetch'
abbr -a fz --set-cursor 'cat $HOME/Projects/private-notes/fzfiles/%.txt | fzf -1 -q "$1" --with-nth 1 --delimiter "\t" --preview \'echo {2} | fold -s -w $(tput cols)\' --bind "enter:execute-silent(wl-copy {2})+accept" 2&>/dev/null; commandline -i (wl-paste)'
